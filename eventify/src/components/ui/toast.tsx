...
// Standalone helper to show the toast without a Ref
// This mimics the 'sonner' toast API but renders our custom component
export const toast = {
  ...
  loading: (message: string, options?: Omit<ToasterProps, 'message' | 'variant'>) => {
  ...
  // Compatibility for generic toast({ ... }) usage
...
// Reusable component extracted from the forwardRef version for use in the static helper
const ToastComponent = ({
  ...
  highlightTitle,
  icon,
}: ToasterProps & { icon?: React.ReactNode }) => {
  const Icon = variantIcons[variant];
  return (
    <motion.div
      variants={toastAnimation}
    ...
    >
      <div className="flex items-start gap-2">
        {icon || (Icon && <Icon className={cn('h-4 w-4 mt-0.5 flex-shrink-0', iconColor[variant])} />)}
        <div className="space-y-0.5">
          {title && (
            <h3
              className={cn(
