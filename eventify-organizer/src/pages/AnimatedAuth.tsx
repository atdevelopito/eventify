...
function AnimatedAuth() {
    ...
    const { user, role, isOrganizer, loading: authLoading, login } = useRole();
    ...
    useEffect(() => {
        if (!authLoading && user) {
            ...
            const from = nextParam || (location.state as any)?.from?.pathname || (role === 'admin' || isOrganizer ? '/organizer' : '/');
            navigate(from, { replace: true });
        }
    }, [user, role, isOrganizer, authLoading, navigate, location]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Check if the error has a response (Server responded with 4xx/5xx)
            const serverMessage = error.response?.data?.message;
            // Check if the request was made but no response received (Network error)
            const networkError = error.request ? 'Server unreachable or network error' : error.message;

            const msg = serverMessage || networkError || 'Something went wrong';
            ...
            <div className="flex items-center justify-center p-8 bg-background">
                <div className="w-full max-w-[420px]">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        {error && (
                            <div className="p-3 text-sm text-red-500 bg-red-500/10 border border-red-500/20 rounded-lg flex flex-col gap-1">
                                <span>{error}</span>
                        ...
                        <Button
                            ...
                            disabled={isLoading}
                        >
                            {isLoading ? "Please wait..." : isForgotPassword ? "Send Reset Link" : isLogin ? "Log in" : "Sign up"}
